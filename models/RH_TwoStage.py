...
def setup_data(forecast, Tmax = 432):
...
def run_rh_two_stage_extensive(FR=50, H=48, F=24, R=12, N=6, modeStr="GEFS"):
    global DELTA_HORIZON
    global DELTA_T_STAGE
    global DELTA_T_SECOND_STAGE
    global DELTA_ROLLING
...
#        irma_data = transform_to_forecast(irma_data)
    ...
    if H > F and R <= F:
        ...
        T_set, T_roll, T_labels, scenarios, _ = setup_data(irma_data)
        instance_data = load_florida_network(DELTA_T_MODEL, 0, T_set[-1], partition_network=False, zone=-1)
        instance_name = 'RH_TwoStageExtensive_IRMA_Shortage_FR%i_H%i_F%i_R%i_N%i_%s' % (FR, H, F, R, N, modeStr)
        print(instance_name, '\n', T_roll, len(scenarios))
        shortage_profiles, demand_profiles, cost_profiles, resultsOut = rolling_horizon(T_set, T_roll, T_labels, instance_data, scenarios,
...
if __name__ == '__main__':
    ...
    FR, R, H, F, N, modeStr = None, None, None, None, None, None
    FR = int(args.FR)
    H = int(args.H)
    R = int(args.R)
    F = int(args.F)
    ...
    run_rh_two_stage_extensive(FR, H, F, R, N, modeStr)
