...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 14 22:48:21 2019

@author: haoxiangyang

rolling horizon scheme with a given deterministic data set
"""
...
sys.path.append(path.abspath('/Users/haoxiangyang/Desktop/Git/daniel_diesel/'))
#sys.path.append(path.abspath('/Users/dduque/Dropbox/WORKSPACE/FuelDistModel'))
...
Model Constants
'''
DELTA_HORIZON = 48 # Hours of look ahead 
DELTA_T_MODEL = 1 #Temporal model resolution
DELTA_T_STAGE = 24 #Temporal resolution of the first stage 
DELTA_T_SECOND_STAGE = int(DELTA_HORIZON-DELTA_T_STAGE) #number of DELTA_T_MODEL periods in the last stage
DELTA_ROLLING = 12 #Periods to roll forward in the rolling horizon

 
def random_builder_factory(t_now, T_max, scenarios, DFSC_instnace):
    '''
        ...
        Args:
            ...
            DFSC_instnace (tuple): instance of the problem
        Returns:
    ...
    def DFSC_random_builder():
        intra_ints = [range(t_now, t_now+DELTA_T_STAGE, DELTA_T_MODEL), range(t_now+DELTA_T_STAGE, T_max_RH,  DELTA_T_MODEL)]
        return DFSC_TwoStage_random_builder(T=2, intra_time_periods = intra_ints, sce_forecast = scenarios, DFSC_instance=DFSC_instnace )
    return DFSC_random_builder

def model_builder_factory(t_now, T_max, DFSC_instnace):
    '''
    ...
    def DFSC_two_stage_model_builder(stage):
        if stage == 0: #First stage lasts DELTA_T_STAGE periods
            return DFSC_TwoStage(t_now, t_now+DELTA_T_STAGE, DELTA_T_MODEL, DFSC_instnace)
        elif stage == 1:
            return DFSC_TwoStage(t_now+DELTA_T_STAGE, T_max_RH,  DELTA_T_MODEL, DFSC_instnace)
        else:
...
def two_stage_opt(t, T, instance_data, scenarios, prev_stage_state):
    ...
    for intra_t in range(t, t+DELTA_T_STAGE, DELTA_T_MODEL):
        shortage_intra_t = sum(model_t.model.getVarByName('%s[%i,%s]' %(pefromance_var_name[0], intra_t, c)).X for c in demand_nodes)
        ...
        shortage_intra_t = sum(model_t.model.getVarByName('%s[%i,%s]' %(pefromance_var_name[1], intra_t, c)).X for c in demand_nodes)
...
def two_stage_opt_extensive(t, T, instance_data, scenarios, prev_stage_state):
    ...
    DFSC_instance = instance_data
    m, in_state, out_state, rhs_vars, out_in_map = DFSC_TwoStage_extensive( t,
                                                                            t + DELTA_T_STAGE, 
                                                                            t + DELTA_T_STAGE + DELTA_T_SECOND_STAGE, 
                                                                            DELTA_T_MODEL, 
                                                                            DELTA_ROLLING, 
                                                                            DFSC_instance, 
                                                                            scenarios)
    ...
    for v in m.getVars():
        if not ('zS' in v.VarName or 'zN' in v.VarName or  
                'I[' in v.VarName or '2[' in v.VarName or 
                'demand' in v.VarName or 'I_intra[' in v.VarName):
            v.lb = v.X
            v.ub = v.X
            #print(v.VarName, v.X)
        else:
    ...
    nominal_demand = DFSC_instance[-1]
    for intra_t in range(t, t+DELTA_ROLLING, DELTA_T_MODEL):
        shortage_intra_t = sum(m.getVarByName('%s[%i,%s]' %(pefromance_var_name[0], intra_t, c)).X for c in demand_nodes)
        performance_out[0].append(shortage_intra_t)
        demand_t =  sum(scenarios[0][intra_t][intra_t][c] for c in demand_nodes) if( intra_t in scenarios[0]) else 0
        perfromance_base[0].append(demand_t)
        
        shortage_intra_t = sum(m.getVarByName('%s[%i,%s]' %(pefromance_var_name[1], intra_t, c)).X for c in demand_nodes)
        performance_out[1].append(shortage_intra_t)
        demand_t =  sum(nominal_demand[intra_t,c] for c in demand_nodes)
        perfromance_base[1].append(demand_t)
...
def setup_data(forecast):
    ...
    pred1.sort()
    DELTA_T_SECOND_STAGE = np.minimum(DELTA_T_SECOND_STAGE,int((pred1[-1] - pred1[0]).days)*24 + int((pred1[-1] - pred1[0]).seconds/3600))
    total_hours = int((prediction_times[-1] - prediction_times[0]).days)*24 + int((prediction_times[-1] - prediction_times[0]).seconds/3600)
    ...
    nfor = {w:{ts_map[it]:{ts_map[pt]:forecast[w][it][pt]  for pt in forecast[w][it]} for it in forecast[w]} for w in forecast}
        ...
        for la in Look_Aheads:
            for first_stage_delta in Operation_fex:
                if la>first_stage_delta:
                    ...
                    instance_name = 'RH_TwoStage_IRMA_Shortage_FR%i_H%i_FS%i_SS%i' %(FR, DELTA_HORIZON, DELTA_T_STAGE, DELTA_T_SECOND_STAGE)
...
def run_rh_two_stage_extensive(FR,R,H,F,irma_data):
    global DELTA_HORIZON
    global DELTA_T_STAGE
    global DELTA_T_SECOND_STAGE
    global DELTA_ROLLING
    ...
    '''
    =================================================================
    Reading deterministic data directly from input
    =================================================================
    '''
    ...
    if H>F and R<=F:
        ...
        DELTA_T_SECOND_STAGE = DELTA_HORIZON - DELTA_T_STAGE
        T_set, T_roll, T_labels, scenarios, _ = setup_data(irma_data)
        # here: should the end of the horizon be T_set[-1] + 1 so that the range will not go over
        instance_data = load_florida_network(DELTA_T_MODEL, 0 , T_set[-1], partition_network=False, zone = -1)
        instance_name = 'RH_TwoStageExtensive_IRMA_Shortage_FR%i_R%i_H%i_FS%i_SS%i' %(FR, DELTA_ROLLING, DELTA_HORIZON, DELTA_T_STAGE, DELTA_T_SECOND_STAGE)
        print(instance_name, '\n', T_roll, len(scenarios))
        shortage_profiles, demand_profiles = rolling_horizon(T_set, T_roll, T_labels, instance_data, scenarios, two_stage_opt_extensive, two_stage_results_processor)
        save_obj = (shortage_profiles, demand_profiles,T_set, T_labels)
        with open('%s/output/%s.p' %(project_path, instance_name), 'wb') as fp:
            pickle.dump(save_obj, fp, protocol=pickle.HIGHEST_PROTOCOL)
...
def decorate_plot(ax,y_title, y_lim, y_ticks=6, f_size = 10, leyend=False):
    ax.set_ylim(0,y_lim)
...
def plot( ):     
    ...
    axR2 = ax2.twinx()
    FR = 20
    H_set = [int(24 * i) for i in [1,2,4]]#0.5, 1,2,
    '''========================================================
                            READ DATA
       ========================================================'''
    results = {}
    ...
    for H in H_set:
        for F in F_set:
            S = H - F
            for R in F_set:
                if H==96: # int(0.5*H)==F and R==F:
                    try:
                        
                        instance_name = 'RH_TwoStageExtensive_IRMA_Shortage_FR%i_R%i_H%i_FS%i_SS%i' %(FR, R, H, F, S)
                        load_obj = None
                        with open('%s/output/%s.p' %(project_path, instance_name), 'rb') as fp: 
                            load_obj = pickle.load(fp)
                        results[(H,R,F,S)] = load_obj
    ...
    for (H,R,F,S) in results:
        instance_name = 'RH_TwoStageExtensive_IRMA_Shortage_FR%i_R%i_H%i_FS%i_SS%i' %(FR, R, H, F, S)
        load_obj = results[H,R,F,S]
        ...
        ax1.plot(real_stamps, cummulative_surge_shortage, color=plot_color, linestyle='--', dashes=(1,1) ,label='R%i_H%i_F%i_S%i' %(R, H, F, S))
        decorate_plot(ax1, y_title='Shortage (Barrels)', y_lim=50000, f_size=my_font_size, leyend=True )
        axR1.plot(real_stamps, cummulative_surge_demand, color='blue', label='Cumulative demand')
        decorate_plot(axR1, y_title='Demand (Barrels)', y_lim=y_lim_surge, f_size=my_font_size)
        fig1.tight_layout()
        
        if surge_demand_plot==False: #Just surge demand plot
            surge_demand_plot = True
            fig, ax = plt.subplots(figsize=(8, 4), dpi=300) 
            ax.plot(real_stamps, cummulative_surge_demand, color='blue', label='Cumulative demand')
            decorate_plot(ax, y_title='Surge demand', y_lim=y_lim_total, y_ticks=11, f_size=my_font_size)
            filename = '%s/output/RH_TwoStage_IRMA_SurgeDemand_FR%i.pdf' %(project_path, FR)
        ...
        real_stamps = T_labels[:min_lenth]                      
        ax2.plot(real_stamps, cummulative_nominal_shortage, color=plot_color, linestyle='--', dashes=(1,1) ,label='R%i_H%i_F%i_S%i' %(R, H, F, S))
        decorate_plot(ax2, 'Nominal shortage (Barrels)', y_lim=1000000, f_size=my_font_size,leyend=True)
        axR2.plot(real_stamps, cummulative_nominal_demand, color='blue', label='Cumulative demand')
        decorate_plot(axR2, y_title='Demand (Barrels)', y_lim=y_lim_nominal, f_size=my_font_size)
        fig2.tight_layout()
        ...
        if nominal_demand_plot==False: #Just nominal demand plot
            ...
            fig, ax = plt.subplots(figsize=(8, 4), dpi=300) 
            ax.plot(real_stamps, cummulative_nominal_demand, color='blue', label='Cumulative demand')
            decorate_plot(ax, y_title='Nominal demand', y_lim=y_lim_total, y_ticks=11 , f_size=my_font_size)
            filename = '%s/output/RH_TwoStage_IRMA_NominalDemand_FR%i.pdf' %(project_path, FR)
            fig.savefig(filename)
            #Total demand plot 
            fig, ax = plt.subplots(figsize=(8, 4), dpi=300) 
            
            ax.plot(real_stamps, total_demand, color='blue', label='Cumulative demand')
            decorate_plot(ax, y_title='Total demand', y_lim=y_lim_total,  y_ticks=11 ,f_size=my_font_size)
            filename = '%s/output/RH_TwoStage_IRMA_TotalDemand_FR%i.pdf' %(project_path, FR)
            fig.savefig(filename)
        
        #shortage plot
        total_shortage = cummulative_surge_shortage + cummulative_nominal_shortage
        ax3.plot(real_stamps, total_shortage, color=plot_color, linestyle='--', dashes=(1,1) ,label='R%i_H%i_F%i_S%i' %(R, H, F, S))
        decorate_plot(ax3, 'Total shortage (Barrels)', y_lim= y_lim_total, y_ticks=11 , f_size=my_font_size, leyend=True)
        axR3.plot(real_stamps, total_demand, color='blue', label='Cumulative demand')
        decorate_plot(axR3, 'Total shortage (Barrels)', y_lim= y_lim_total, y_ticks=11 , f_size=my_font_size)
        fig3.tight_layout()
        
        plot_id = plot_id + 1

    
    filename = '%s/output/RH_TwoStage_IRMA_SurgeShortage_FR%i.pdf' %(project_path, FR)
    fig1.savefig(filename)
    filename = '%s/output/RH_TwoStage_IRMA_NominalShortage_FR%i.pdf' %(project_path, FR)
    fig2.savefig(filename)
    filename = '%s/output/RH_TwoStage_IRMA_TotalShortage_FR%i.pdf' %(project_path, FR)
